...
"""
mutation nueron for NOR and mutation layer for NOR are normal; they are only used in the final layer,
where the state size is small enough to allow a neuron with unrestricted strategies
"""
class mutation_neuron_for_NOR:
    """
...
class mutation_layer_for_NOR:
    ...
    def initialise_neurons(self):
        ...
        for i in range(self.num_neurons):
            neuron_list.append(mutation_neuron_for_NOR(input_size=self.input_size))
        return neuron_list
...
class restricted_NOR_neuron:
    ...
    def get_activation(self, state):
        ...
        if strategy_number == self.num_strategies - 1:
            return 0, strategy_number  #zero strategy for NOR neuron
        else:
...
class restricted_NOR_layer:
    ...
    def initialise_neurons(self):
        neuron_list = []
        for i in range(self.num_neurons):
            neuron_list.append(restricted_NOR_neuron(input_size=self.input_size))
        return neuron_list
    ...
    def initialise_network(self):
        ...
        for i in range(self.num_layers):
            if i == self.num_layers - 1:
                layer = mutation_layer_for_NOR(num_neurons=1,
                                               input_size=self.topology[i])
            else:
                layer = restricted_NOR_layer(num_neurons=self.neurons_in_each_layer[i],
                                             input_size=self.topology[i])
            Layers.append(layer)
        return Layers
